...
def software_lookup(file_objects, filename, firmwares):
    firmware_ids = []
    ...
    for i in file_objects.find({"file_name": query}):
        if 'cve_lookup' in i['processed_analysis'] and 'cve_results' in i['processed_analysis']['cve_lookup'] and len(
            ...
            for j in i['parent_firmware_uids']:
                if j not in firmware_ids:
                    firmware_ids.append(j)
    print("Total number of firmwares with software component \"" + filename + "\": " + str(len(firmware_ids)))
    results = []
    for i in firmware_ids:
        query2 = {"_id": i}
        data = firmwares.find_one(query2)
        results.append("{} {} {}".format(data["vendor"], data["device_name"], data["version"]))
    results.sort()
    for i in results:
        print(i)
...
def alt2_count_cve(file_objects):
    ...
    unique = set(i.strip().rsplit(' ', 1)[0] for i in data)
    print(unique)
...
def main():
    ...
    firmwares = db.firmwares
    if args.lookup != 'null':
        software_lookup(file_objects, args.lookup, firmwares)
    if args.count:
